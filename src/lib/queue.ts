...
function getRetryDelay(attempt: number): number {
  return Math.min(30000 * Math.pow(4, attempt), 480000);
}
...
export async function failJob(jobId: string, error: string, attempts: number, maxAttempts: number): Promise<boolean> {
  ...
  await supabase
    ...
    .update({
      ...
      attempts,
      updated_at: new Date().toISOString(),
      next_retry_at: shouldRetry ? new Date(Date.now() + getRetryDelay(attempts)).toISOString() : null,
    })
    .eq("id", jobId);

  return !shouldRetry; // Returns true if permanently failed
}

export async function getJobStats(): Promise<{ pending: number; processing: number; failed: number; completed_today: number }> {
  const supabase = await createClient();
